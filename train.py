...
def get_args():
    ...
    parser.add_argument("--work_dir", type=str, help="working directory",default='/home/jesse/Project/SymPoint/data/out/train01')
...
def train(
    epoch, model, optimizer, scheduler, scaler, train_loader, cfg, logger, writer
):
    model.train()
    ...
    for i, batch in enumerate(train_loader, start=1):
        ...
        if scheduler is None:
            cosine_lr_after_step(
                optimizer, cfg.optimizer.lr, epoch - 1, cfg.step_epoch, cfg.epochs
            )
...
def main():
    ...
    logger.info(
        "Total Number of Parameters: {} M".format(str(float(total_params) / 1e6)[:5])
    )
    logger.info(
        "Total Trainable Number of Parameters: {} M".format(
            str(float(trainable_params) / 1e6)[:5]
        )
    )
    ...
    train_loader = build_dataloader(
        args, train_set, training=True, dist=args.dist, **cfg.dataloader.train
    )
    ...
    logger.info(
        f"Scale LR from {default_lr} (batch size 16) to {scaled_lr} (batch size {total_batch_size})"
    )
